#!/usr/bin/env python3
import pytest
from backendProject.views.blog import get_post, update, delete, create, index
...
def test_index(client, auth):
    ...
    assert b"by test_name1 on 2018-01-01" in response.data
...
def test_author_required(app, client, auth):
    ...
    with app.app_context():
        db = get_db()
        db.execute("UPDATE post SET client_id = 2 WHERE id = 1")
        db.commit()
